...
interface Campaign{id:string;name:string;status:string;recipient_count:number;queued_at:string}
interface Delivery{id:string;event:string;channel:string;recipient:string;status:string;sent_at?:string;error?:string}
const api=useTailorsApi(),templates=ref<Template[]>([]),campaigns=ref<Campaign[]>([]),deliveries=ref<Delivery[]>([]),error=ref(''),message=ref(''),template=reactive({event:'order.ready',channel:'sms',name:'Order ready',body:'Hello {{customer_name}}, your order {{order_number}} is ready.',active:true}),campaign=reactive({name:'',channel:'sms',body:'Hello {{customer_name}},',consent_confirmed:false})
async function load(){const result=(await api.request<{templates:Template[];campaigns:Campaign[];deliveries:Delivery[]}>('/notifications')).data;templates.value=result.templates;campaigns.value=result.campaigns;deliveries.value=result.deliveries}
...
<template>
  <header>
    ...
    <form class="panel form" @submit.prevent="saveTemplate">
      ...
      <h2>Occasion campaign</h2><label>Name<input v-model="campaign.name" required></label><label>SMS message<textarea v-model="campaign.body" required /></label><label class="check"><input v-model="campaign.consent_confirmed" type="checkbox" required> Send only to customers with recorded marketing consent</label><button class="primary">
        Queue campaign
      </button>
    </form>
