...
class AppController extends Controller
{
    ...
    public function dashboard(): JsonResponse
    {
        ...
        return $this->ok(['metrics' => ['due_today' => Order::whereDate('promised_at', $day)->whereNotIn('status', ['delivered', 'cancelled'])->count(), 'in_progress' => Order::where('status', 'in_progress')->count(), 'ready' => Order::where('status', 'ready')->count(), 'revenue_minor' => Order::whereDate('created_at', $day)->sum('paid_minor')], 'recent_orders' => Order::with(['customer', 'garment', 'karigar'])->latest('id')->limit(8)->get()->map(fn ($o) => $this->order($o)), 'onboarding' => [
